    ...
    )
{
    ...
    {
        DHCPMGR_LOG_DEBUG("event/function name should not be null!\n");
...
EvtDispterEventListen(void)
{
    ...
    if(retval) {
        ...
        if (SE_MSG_NOTIFICATION == msg_type) {
            ...
            DHCPMGR_LOG_DEBUG("Received msg that is not a SE_MSG_NOTIFICATION (%d)\n", msg_type);
            if ( 0 != system("pidof syseventd")) {

                DHCPMGR_LOG_DEBUG("%s syseventd not running ,breaking the receive notification loop \n",__FUNCTION__);
                ret = EVENT_HANDLE_EXIT;
           }
        }
    } else {
        DHCPMGR_LOG_DEBUG("%s: Received no event retval=%d\n", __FUNCTION__, retval);
    }
    return ret;
}
...
EvtDispterEventHandler(void *arg)
{
    ...
    {
        DHCPMGR_LOG_DEBUG("%s: sysevent init failed!\n", __FUNCTION__);
        sleep(1);
    }
    ...
    while(1)
    {
        ...
        switch (ret)
        {
            ...
            default :
                DHCPMGR_LOG_DEBUG("The received event status is not expected!\n");
                break;
        }
...
EvtDispterHandleEventAsync(void)
{
    ...
    if(0 != err)
    {
        DHCPMGR_LOG_DEBUG("%s: create the event handle thread error!\n", __FUNCTION__);
    }
}
...
int executeCmd(char *cmd)
{
    ...
    {
        DHCPMGR_LOG_DEBUG("%s: %s command didnt execute successfully\n", __FUNCTION__,cmd);
